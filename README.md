...
```bash
python generate_addon_data.py <build>
```
...
Tests the algorithm against game-extracted data in `test_data/`. Supports testing individual algorithm implementations:

```bash
python run_tests.py <build> -a dbc    # Direct DBC algorithm only
python run_tests.py <build> -a addon  # Addon data algorithm only
python run_tests.py <build> -a lua    # Lua algorithm only
```
